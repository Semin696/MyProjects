...
public class ReloadCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
        if (!plugin.hasPermission(sender, "plreloader.reload")) return true;

        if (args.length == 0) {
            sender.sendMessage(plugin.getMessage("usage-reload"));
            return true;
        }
        ...
        sender.sendMessage(plugin.getMessage("plugin-reloaded", "plugin", pluginName));
        return true;
    }
}
