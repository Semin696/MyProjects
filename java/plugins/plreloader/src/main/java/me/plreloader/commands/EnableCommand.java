...
public class EnableCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
        if (!plugin.hasPermission(sender, "plreloader.enable")) return true;

        if (args.length == 0) {
            sender.sendMessage(plugin.getMessage("usage-enable"));
            return true;
        }
        ...
        if (target.isEnabled()) {
            sender.sendMessage(plugin.getMessage("plugin-already-enabled", "plugin", pluginName));
            return true;
        }

        pm.enablePlugin(target);
        sender.sendMessage(plugin.getMessage("plugin-enabled", "plugin", pluginName));
        return true;
    }
}
